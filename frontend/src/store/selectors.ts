...
export const getPossibleOrganizers = (eventId: number) => (state: any) => {
	...
	return getOrdinaryMembers(state)
		.filter((organizer: Person) => !organizerIds.includes(organizer.id!))
		.sort(comparePeopleWithoutStatuses);
};
...
export const getPossibleGuests = (eventId: number) => (state: any) => {
	...
	return getInvitedMembers(state)
		.filter((organizer: Person) => !guestIds.includes(organizer.id!))
		.sort(comparePeopleWithoutStatuses);
};
...
export const getEventsData = (state: any) => state.gotEventsData;
