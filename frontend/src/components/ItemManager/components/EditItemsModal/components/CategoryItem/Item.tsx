...
import {binIcon, checkIcon, crossIcon, penIcon} from "../../../../../../common/icons/icons";
...
export const Item: FC<{item: IItem}> = ({item}) => {
    ...
    return <div className="d-flex flex-column gap-1">
        <div className='category-item d-flex gap-2'>
            <Input refCallback={refCallback} disabled={!edit} style={{width: '100%'}} type="text" onChange={changeHandler(setName, () => setError(""))} value={name}/>
            <div className={`d-flex gap-1 ${edit ? "hidden-btn-group" : "hiding-btn-group"}`}>
                <button onClick={toggleEdit} className="empty pen">{penIcon(24, 'blue')}</button>
                <button onClick={deleteObj} className="empty bin">{binIcon(24, 'red')}</button>
            </div>
            <div className={`d-flex gap-1 ${edit ? "" : "hidden-btn-group"}`}>
                <button onClick={updateObj} className="empty check">{checkIcon(26, '#31c410')}</button>
                <button onClick={closeEdit} className="empty bin">{crossIcon(20, 'red')}</button>
            </div>
        </div>
