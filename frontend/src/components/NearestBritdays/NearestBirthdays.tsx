...
const NearestBirthdaysWithHeaders = ({birthdays}: { birthdays: PersonBirthday[] }) => {
    const headerTitle = {
    ...
    const [nearestBirthdayDate, setNearestBirthdayDate] = useState<Date | null>(null);
    ...
    return (
        <>
            <div>
                ...
                {nearestBirthdayDate &&
                    <p style={headerTitle}>{`Birthdays on ${nearestBirthdayDate.toLocaleDateString('default', {month: 'long'})} ${nearestBirthdayDate.getDate()}`}</p>}
...
const BirthdayTable = ({birthdays}: { birthdays: PersonBirthday[] }) => {
    return (
        <div className="d-flex flex-column gap-2">
            {
                birthdays.map(birthday => <BirthdayRow key={birthday.email} date={birthday.birthday}
                                                       name={birthday.name} email={birthday.email}/>)
            }
        </div>
    );
}

const BirthdayRow = ({name, date, email}: { name: string, date: Date, email: string }) => {
    ...
    return (
        <div className="d-flex align-items-center">
            <div style={{width: "80px"}}>
                <img className="rounded-circle mw-100 h-auto"
                     src={imgUrl}
                     alt=""/>
            </div>
            <div className="ms-3">
