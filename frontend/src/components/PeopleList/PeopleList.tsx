...
import {getAllPeople, getFullName, Person, statusesForDropdown} from '../../api/person';
...
export const PeopleList: FC<{gotData: number, setGotData: Dispatch<SetStateAction<number>>}> =
    ({gotData, setGotData}) => {
        ...
        const [possibleGenerations, setPossibleGenerations] = useState<Array<Generation>>([]);
        ...
        const suitsFilter = (person: Person) => {
            ...
            if (filterStatuses.length === 0 || filterStatuses.length === 5) suitable = true;
            else if (!filterStatuses.includes(person.status))
                return false;

            if (filterYears.length === 0 || filterYears.length === possibleYears.length) suitable = true;
            else if (!person.year_enter && filterYears.includes('Не встановлено')) suitable = true;
            else if (!person.year_enter || !filterYears.includes(person.year_enter.toString()))
                return false;

            if (filterHasParent.length === 0 || filterHasParent.length === 2) suitable = true;
            else if (filterHasParent.includes('Немає') && person.parent)
                return false;
            else if (filterHasParent.includes('Є') && !person.parent)
                return false;

            if (filterGenerations.length === 0 || filterGenerations.length === possibleGenerations.length) suitable = true;
            else if (!person.generation && filterGenerations.includes(undefinedGeneration)) suitable = true;
            else if (!person.generation || !filterGenerations.includes(person.generation))
                return false;
        ...
        const people = useSelector((state: any) =>
            state.people.filter((person: Person) => suitsSearch(person) && suitsFilter(person))
        );
            ...
            async function fetchData() {
                ...
                if (peopleRes) {
                    ...
                    peopleRes.forEach((person) => {
                        if (person.year_enter && !yearsCopy.includes(person.year_enter.toString())) {
                            yearsCopy.push(person.year_enter.toString());
                        }
                        if (person.generation && !generationsCopy.includes(person.generation)) {
        ...
        const updateFilters = (param: string) => {
            ...
            if (vals.length !== 0) {
                if (param === 'status') {
                    ...
                    setFilterYears(vals);
                else if (param === 'hasParent')
                    setFilterHasParent(vals);
        ...
        return (
            !localStorage.getItem('token') ? (
                ...
                (<>
                    ...
                    <Button variant='primary' onClick={addPerson} id='addPerson' className='ms-4 m-2 align-self-start'>Додати людину</Button>
                    <SearchBar searchText={searchText} setSearchText={setSearchText} placeholder='Пошук людини...'/>
                    <Row>
                        <Col>
                            <DropdownWithCheckboxes id='dropdownMenuStatus' name='status' labelText='Статуси' items={statusesForDropdown.map((s) => s.ukrMany)} updateFilters={updateFilters}/>
                        </Col>
                        <Col>
                            <DropdownWithCheckboxes id='dropdownMenuYear' name='year' labelText='Роки вступу в КМА' items={possibleYears} updateFilters={updateFilters}/>
                        </Col>
                        <Col>
                            <DropdownWithCheckboxes id='dropdownMenuHasParent' name='hasParent' labelText='Наявність патрона' items={['Є', 'Немає']} updateFilters={updateFilters}/>
                        </Col>
                        <Col>
                            <DropdownWithCheckboxes id='dropdownMenuGeneration' name='generation' labelText='Покоління' items={possibleGenerations.map((g) => g.name)} updateFilters={updateFilters}/>
                        </Col>
                    </Row>
